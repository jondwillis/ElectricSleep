...
public abstract class XYChart extends AbstractChart {
	...
	public void draw(final Canvas canvas, final int x, final int y,
		...
		final int top = y;
		int right = x + width;
		int bottom = y + height - legendSize - 10;
		if (screenR == null) {
			screenR = new Rect();
		}
		...
		if (showLabels || showGrid) {
			...
			if (showLabels) {
				...
				if (xLabels.size() > 0) {
					bottom -= paint.getTextSize();
				}
				if (yLabels.size() > 0) {
					left += paint.getTextSize();
				}
			}
			...
			for (int i = 0; i < length; i++) {
				...
				if (or == Orientation.HORIZONTAL) {
					if (showLabels) {
						...
						drawText(canvas, getLabel(label), left - 10,
								yLabel - 10, paint, 0);
					}
					if (showGrid) {
			...
			if (showLabels) {
				...
				if (or == Orientation.HORIZONTAL) {
					drawText(canvas, mRenderer.getXTitle(), x + width / 2,
							bottom + xTitleSize - padding, paint, 0);
					drawText(canvas, mRenderer.getYTitle(), x + yTitleSize, y
							+ height / 2, paint, -90);
					...
					drawText(canvas, mRenderer.getXTitle(), x + width / 2, y
							+ height - bottom + xTitleSize - padding, paint,
							-90);
					drawText(canvas, mRenderer.getYTitle(), right + bottom
							+ yTitleSize, y + height / 2, paint, 0);
					paint.setTextSize(mRenderer.getChartTitleTextSize());
